...
!     ---------- ----
      ...
      DOUBLE PRECISION TC1, TA1, TP1, TS1, b1, b2, b3, b4, b5, b6, b7, Kt2, Kts2, Kn2, Ks2, Kd2
        ...
        Es = 0.525d6     ! Symmetric freshwater flux [m3s-1]
        ...
        ! State the ODEs
        F(1) = ((QS1*Sts+QS2*St)+qu*Sd-QN1*St+rs*(Sts-St)+rn*(Sn-St)+2*Es)*t_dim/Vt-(St*(qu+qEk-qe-QN1)*t_dim/(Ar*D_dim))/D                                     ! Salinity thermocline box
        F(2) =  (qEk*Ss-qe*Sts-(QS1*Sts+QS2*St)+rs*(St-Sts))*t_dim/Vts-(Sts*(qu+qEk-qe-QN1)*t_dim/(Ar*D_dim))/D                                            ! Salinity ts box
        F(3) = ((QN1+rn)*(St-Sn)-(Es+Ea*1e6))*t_dim/Vn            ! Salinity northern box
        F(4) = ((QS1*Sd+QS2*Ss)+qe*Sts-qEk*Ss-(Es-Ea*1e6))*t_dim/Vs!((QS1*Sd+QS2*Ss)+qe*Sts-qEk*Ss-(Es-Ea*1e6))*t_dim/Vs   ! Salinity southern box
        F(5) = (qu+qEk-qe-QN1)*t_dim/(Ar*D_dim)                  ! Thermocline depth

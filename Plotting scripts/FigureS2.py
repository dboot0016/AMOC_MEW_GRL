...
#%% Load in branch 1
run2_1 = pd.read_csv(f'{Datadir}/'+'b.es2056_0_branch_1',sep='\s+',header=None)   
run1_1 = pd.read_csv(f'{Datadir}/'+'b.bio056_0_branch_1',sep='\s+',header=None)  
run1_2 = pd.read_csv(f'{Datadir}/'+'b.bio056_0_branch_2',sep='\s+',header=None)
run2_2 = pd.read_csv(f'{Datadir}/'+'b.es2056_0_branch_2',sep='\s+',header=None)

for jj in range(5): 
    # Branch 1
    run3_1 = pd.read_csv(f'{Datadir}/'+'b.s'+str(jj+1)+'_branch1',sep='\s+',header=None)   
    run4_1 = pd.read_csv(f'{Datadir}/'+'b.s'+str(jj+6)+'_branch1',sep='\s+',header=None)    
    
    # Branch 2  
    run3_2 = pd.read_csv(f'{Datadir}/'+'b.s'+str(jj+1)+'_branch2',sep='\s+',header=None)   
    run4_2 = pd.read_csv(f'{Datadir}/'+'b.s'+str(jj+6)+'_branch2',sep='\s+',header=None)   
    ...
    Sd1_2 = (S_dim*V0 - St1_2*(A*D1_2)-Sts1_2*(LxA*Ly*D1_2)-Ss1_2*Vs-Sn1_2*Vn)/(V0-Vn-Vs-LxA*Ly*D1_2-A*D1_2)
    ...
    qn1_2 = 1e-6*(eta*alpha*(Tts-Tn)*(D1_2)**2+eta*beta*(Sn1_2-Sts1_2)*(D1_2)**2)
...
#%% Figure S2
    ...
    for i in np.arange(0,np.size(x1),1):
        if np.array(run1_1[1])[i]>0:
           ...
           LS='solid'    
        plt.plot(x1[i:i+2],y1[i:i+2],linewidth=LW,color='black',linestyle = LS,label='_nolegend_')
    
    for i in np.arange(0,np.size(x2),1):
        if np.array(run1_2[1])[i]>0:
            LS='dotted'
        else:
           LS='solid'    
        plt.plot(x2[i:i+2],y2[i:i+2],linewidth=LW,color='black',linestyle = LS,label='_nolegend_')
    ...
    for i in np.arange(0,np.size(x3),1):
        if np.array(run2_1[1])[i]>0:
           ...
           LS='solid'    
        plt.plot(x3[i:i+2],y3[i:i+2],linewidth=LW,color='tab:orange',linestyle = LS,label='_nolegend_')
    ...
    for i in np.arange(0,np.size(x4),1):
        if np.array(run2_2[1])[i]>0:
           ...
           LS='solid'    
        plt.plot(x4[i:i+2],y4[i:i+2],linewidth=LW,color='tab:orange',linestyle = LS,label='_nolegend_')    
    ...
    for i in np.arange(0,np.size(x5),1):
        if np.array(run3_1[1])[i]>0:
           ...
           LS='solid'    
        plt.plot(x5[i:i+2],y5[i:i+2],linewidth=LW,color='tab:blue',linestyle = LS,label='_nolegend_')
    ...
    for i in np.arange(0,np.size(x6),1):
        if np.array(run3_2[1])[i]>0:
           ...
           LS='solid'    
        plt.plot(x6[i:i+2],y6[i:i+2],linewidth=LW,color='tab:blue',linestyle = LS,label='_nolegend_')
    ...
    plt.plot([-1e5,-2e5],[np.mean(y1),np.mean(y5)],linewidth=LW,color='black',label=label1)
    plt.plot([-1e5,-2e5],[np.mean(y1),np.mean(y5)],linewidth=LW,color='tab:orange',label=label2)
    plt.plot([-1e5,-2e5],[np.mean(y1),np.mean(y5)],linewidth=LW,color='tab:blue',label=label3)
    ...
    if save_fig == 'yes':
        if vary=='D':
            plt.savefig('figureS_AMOC_'+str(jj+1)+'_'+str(jj+6)+'.png', format='png', dpi=quality, bbox_inches = 'tight')
        elif vary=='CO$_2$':
            plt.savefig('figureS_CO2_'+str(jj+1)+'_'+str(jj+6)+'.png', format='png', dpi=quality, bbox_inches = 'tight')
